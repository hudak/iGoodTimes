...
- The React app talks to this instance via a `VITE_POCKETBASE_URL` environment variable set at build time. **Not via Cloudflare Pages' dashboard/API "environment variables" feature** — found and confirmed a real bug there: setting `VITE_POCKETBASE_URL` through `deployment_configs.production.env_vars` (the same field the dashboard's Settings → Environment variables writes to) never actually reached the build process. Proof: two separate rebuilds with that config set produced a bundle with an identical content hash to a build from *before* the var existed at all — while inlining the exact same value directly into the build command (`VITE_POCKETBASE_URL=... pnpm build`) produced a different hash with the value correctly present. Root cause not identified beyond that; worked around by moving the frontend deploy to GitHub Actions running `wrangler pages deploy` directly (see CI/CD below), where the var is a normal step `env:` value with no ambiguity about whether it reaches the build.
...
- A second, parallel workflow (`.github/workflows/deploy-frontend.yml`, same path-filtered-on-push-to-`main` shape) builds the frontend and deploys it with `wrangler pages deploy beach-weeks/dist --project-name=goodtimes`, authenticated via `CLOUDFLARE_API_TOKEN`/`CLOUDFLARE_ACCOUNT_ID` repository secrets.
- This replaces Cloudflare Pages' own git-integration auto-builds, which are now disabled on the project (`production_deployments_enabled: false`) — both because of the environment-variable bug above and so there's one obvious place (GitHub Actions) that owns deploying both halves of the app, matching the Railway side.
- `VITE_POCKETBASE_URL` is a plain workflow-level `env:` value (not a secret — it's a public API URL) read directly by the `pnpm build` step, sidestepping the dashboard/API env var path entirely.
