...
- [ ] 5.4 Set the frontend's production `VITE_POCKETBASE_URL` to the Railway instance URL; verify a production build talks to the deployed PocketBase instance — backend is reachable at `https://api.goodtimes.huskytown.net` (custom domain, verified/cert issued, target port fixed to 8090; `meta.appURL` set to match). Set `VITE_POCKETBASE_URL=https://api.goodtimes.huskytown.net` as a production build env var on the Cloudflare Pages project via the API (confirmed this can't go in `wrangler.toml`'s `[vars]` — that only configures Pages *Functions* runtime bindings, not the static build step; Vite needs it present when `vite build` actually runs). **Remaining:** the current production deployment predates all of this change's frontend code (last build was before this session), so the var hasn't been exercised by a real build yet — verify once this branch is pushed and Pages rebuilds
