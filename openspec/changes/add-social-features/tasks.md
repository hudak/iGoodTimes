...
- [ ] 5.4 Set the frontend's production `VITE_POCKETBASE_URL` to the Railway instance URL; verify a production build talks to the deployed PocketBase instance — backend is reachable at `https://api.goodtimes.huskytown.net` (custom domain, verified/cert issued, target port fixed to 8090; `meta.appURL` set to match). **Remaining:** actually setting `VITE_POCKETBASE_URL` on the Cloudflare Pages production build and confirming a real production build talks to it — needs Cloudflare auth I don't have in this session
